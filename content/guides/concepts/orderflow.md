...
Direct offline transaction: the funds are sent to a 1-of-2 multisignature escrow address. The funds are moved to an address controlled by the seller's wallet after the seller accepts the order. If the seller rejects the order, or the buyer cancels it before the seller accepts it, the funds are moved back to the buyer.
